...
def extract_gallery_images_html(soup, base_url_domain):
    ...
    tags = soup.find_all(['img', 'source', 'a'])
    for tag in tags:
...
def collect_gallery_images(url, soup, base_url_domain, session, headers, product_data, max_images):
    """Combine JSON-LD, product.json endpoint, og:image, and HTML gallery extraction,
...
# ============================================================
def scrape_shopify_product(url, session, config, ai_config=None):
...
def process_batch(urls, config, session, ai_config=None):
    ...
    failed = []
    for url in urls:
        results, img_data, error = scrape_shopify_product(url, session, config, ai_config)
        if results:
